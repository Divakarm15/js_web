...
function initSocket() {
  ...
  socket.on('connect', () => {
    ...
    const el = document.getElementById('menubar-status');
    if (el && el.textContent === 'Disconnected') el.textContent = 'Ready';
  });
...
async function startScan() {
  ...
  const options = {
    ...
    depth: parseInt(document.getElementById('opt-depth')?.value) || 2,
    max_clicks: parseInt(document.getElementById('opt-max-clicks')?.value) || 50,
  };
...
function populateResults(results) {
  ...
  if (zipBtn) {
    zipBtn.onclick = () => downloadResultsZip(results.scan_id);
  }

  // Populate Selenium stats tab
  const seleniumEl = document.getElementById('selenium-stats-content');
  if (seleniumEl && results.selenium_stats) {
    const ss = results.selenium_stats;
    seleniumEl.innerHTML = `
      <div class="selenium-stat-grid">
        <div class="selenium-stat-item">
          <span class="selenium-stat-label">Crawl Depth</span>
          <span class="selenium-stat-value">${ss.depth}</span>
        </div>
        <div class="selenium-stat-item">
          <span class="selenium-stat-label">Max Clicks</span>
          <span class="selenium-stat-value">${ss.max_clicks}</span>
        </div>
        <div class="selenium-stat-item">
          <span class="selenium-stat-label">Headless</span>
          <span class="selenium-stat-value">${ss.headless ? 'Yes' : 'No'}</span>
        </div>
        <div class="selenium-stat-item">
          <span class="selenium-stat-label">JS URLs via Selenium</span>
          <span class="selenium-stat-value">${ss.urls_from_selenium || 0}</span>
        </div>
        <div class="selenium-stat-item">
          <span class="selenium-stat-label">Total JS Files</span>
          <span class="selenium-stat-value">${results.unique_files || 0}</span>
        </div>
        <div class="selenium-stat-item">
          <span class="selenium-stat-label">Scan Duration</span>
          <span class="selenium-stat-value">${results.duration}s</span>
        </div>
      </div>
      <div class="selenium-pipeline-steps">
        <div class="pipeline-title">📋 Crawl Pipeline</div>
        <div class="pipeline-step"><span class="step-badge passive">1</span> GAU — passive historical URL discovery</div>
        <div class="pipeline-step"><span class="step-badge active">2</span> Katana — active link crawling</div>
        <div class="pipeline-step"><span class="step-badge active">3</span> Direct HTTP scraper — &lt;script src&gt; extraction</div>
        <div class="pipeline-step"><span class="step-badge selenium">4</span> Selenium + Chromium — click automation, lazy loading, SPA rendering</div>
        <div class="pipeline-step"><span class="step-badge auth">5</span> Playwright — authenticated crawling (if credentials provided)</div>
        <div class="pipeline-step"><span class="step-badge download">6</span> Download all JS files with deduplication</div>
        <div class="pipeline-step"><span class="step-badge js-crawl">7</span> JS File Content Crawl — parse every file for imports, webpack chunks, source maps, manifests</div>
        <div class="pipeline-step"><span class="step-badge analyze">8</span> Security analysis — secrets, endpoints, keywords</div>
      </div>
      <div class="selenium-output-note">
        <strong>Output structure:</strong>
        <code>output/prioritized/</code> — main.js, app.js, bundle.js, large files<br>
        <code>output/other_js/</code> — all other JS files<br>
        <code>output/inline_js/</code> — extracted inline scripts<br>
        <code>output/dynamic_chunks/</code> — webpack/vite chunks<br>
        <code>output/findings_report.json</code> — security findings<br>
        <code>output/metadata.json</code> — scan metadata
      </div>
    `;
  }
...
function downloadResultsZip(scanId) {
  if (scanId) {
    window.location.href = `/api/scan/${scanId}/download`;
  } else {
